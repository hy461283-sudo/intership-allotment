...
export default function StudentRegistration() {
  ...
  const validateStep = (targetStep = step) => {
    ...
    if (targetStep === 1) {
      ...
      if (!formData.currentAddress) temp.currentAddress = "Current Address is required."
      if (!formData.permanentAddress) temp.permanentAddress = "Permanent Address is required."
      if (!formData.photo) temp.photo = "Photo upload required."
      if (!formData.govProof) temp.govProof = "Gov ID Proof upload required."
    }

    if (targetStep === 2) {
      if (!formData.fatherName) temp.fatherName = "Father's name required."
      if (!formData.motherName) temp.motherName = "Mother's name required."
      if (!formData.guardianName) temp.guardianName = "Guardian name required."
      ...
      if (!formData.guardianAddress) temp.guardianAddress = "Guardian address required."
      if (!formData.guardianIdProof) temp.guardianIdProof = "Guardian ID proof upload required."
    }

    if (targetStep === 3) {
      if (!formData.studentId) temp.studentId = "Student ID is required."
      ...
      else if (isNaN(formData.cgpa) || formData.cgpa <= 0 || formData.cgpa > 10) temp.cgpa = "CGPA must be 0-10."
      if (!formData.skills) temp.skills = "Skills are required."
    ...
    if (targetStep === 4) {
      ...
      else if (!passwordPattern.test(formData.password)) temp.password = "Password must contain upper, lower, number and symbol."
      ...
      <div className="w-full max-w-4xl bg-white shadow-lg rounded-lg border p-8">
        ...
        <div className="flex justify-between mb-8">
          {stepsMenu.map((s) => (
            <div key={s.num} className="flex-1 text-center">
              <h4 className={`font-semibold ${step === s.num ? "text-purple-700" : "text-gray-500"}`}>
                Step {s.num}: {s.label}
              </h4>
        ...
        {step === 1 && (
          <form onSubmit={handleNext} className="space-y-6">
            {/* Your original UI section (unchanged) */}
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              <InputField label="Email" name="email" value={formData.email} onChange={handleChange} error={errors.email} required />
              <InputField label="Alternate Email" name="altEmail" value={formData.altEmail} onChange={handleChange} error={errors.altEmail} required />
            </div>
        ...
        {step === 2 && (
          <form onSubmit={handleNext} className="space-y-6">
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              <InputField label="Relation" name="guardianRelation" value={formData.guardianRelation} onChange={handleChange} error={errors.guardianRelation} required />
              <InputField label="Guardian Email" name="guardianEmail" value={formData.guardianEmail} onChange={handleChange} error={errors.guardianEmail} required />
            ...
            <div className="flex justify-between pt-4">
              <button type="button" onClick={handlePrev} className="bg-gray-100 text-gray-700 px-6 py-2 rounded-md hover:bg-gray-200 transition">
                ← Back
              </button>
              <button type="submit" className="bg-purple-700 text-white px-6 py-2 rounded-md hover:bg-purple-800 transition">
                Next: Academic Info →
        ...
        {step === 3 && (
          <form onSubmit={handleNext} className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              <InputField label="CGPA" name="cgpa" value={formData.cgpa} onChange={handleChange} error={errors.cgpa} required />
            </div>

            <TextAreaField label="Skills" name="skills" value={formData.skills} onChange={handleChange} error={errors.skills} required />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full border-2 rounded-md px-3 py-2 pr-10 focus:ring-2 focus:ring-purple-600"
                />
                <button
                  type="button"
            ...
            <div>
              ...
              <input
                ...
                className="w-full border-2 rounded-md px-3 py-2 focus:ring-2 focus:ring-purple-600"
              />
            ...
            <div className="flex justify-between pt-4">
              ...
              <button type="submit" disabled={loading} className="bg-green-600 text-white px-8 py-2 rounded-md hover:bg-green-700 transition">
  ...
  return (
    <div>
      ...
      <input
        ...
        value={value}
        onChange={onChange}
        className={`w-full border-2 rounded-md px-3 py-2 focus:ring-2 focus:ring-purple-500 ${error ? "border-red-500" : "border-gray-300"}`}
      />
